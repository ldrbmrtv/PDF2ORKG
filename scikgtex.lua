...
function XMP:generate_xmp_string(lb_char)
    ...
    if self.paper then
        ...
        --Ildar+
        self:add_line('    <orkg:hasTitle>%s</orkg:hasTitle>', self.paper.title)
        for i, author in ipairs(self.paper.authors) do
          self:add_line('    <orkg:hasAuthor>%s</orkg:hasAuthor>', author)
        end
        self:add_line('    <orkg:hasResearchField>%s</orkg:hasResearchField>', self.paper.researchfield)
        --Ildar-
